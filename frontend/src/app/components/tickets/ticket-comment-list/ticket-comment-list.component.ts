...
export class TicketCommentListComponent implements OnInit {
  ...
  loading = false;
  errorMessage = '';
  constructor(private readonly commentService: TicketCommentService, private readonly toast: ToastService, private readonly route: ActivatedRoute) {}
  ...
  deleteComment(id: number): void { this.commentService.delete(id).subscribe({ next: () => { this.comments = this.comments.filter((item) => item.id !== id); this.toast.success('Comment deleted successfully.'); }, error: (error) => this.toast.error(error, 'Unable to delete comment.') }); }
}
