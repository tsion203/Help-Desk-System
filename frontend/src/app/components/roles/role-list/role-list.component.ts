...
export class RoleListComponent implements OnInit {
  ...
  async deleteRole(id: number, event: Event): Promise<void> { event.stopPropagation(); const item=this.roles.find(value=>value.id===id); const result=await this.confirmation.confirm({title:'Delete role?',message:`Delete ${item?.name || `role #${id}`}? This action cannot be undone.`,confirmText:'Delete role',danger:true}); if(!result.confirmed)return; this.roleService.delete(id).subscribe({ next: () => {if(this.roles.length===1&&this.page>0)this.page--;this.loadPage();this.toast.success('Role deleted successfully.'); }, error: (error) => this.toast.error(error, 'Unable to delete role.') }); }
}
