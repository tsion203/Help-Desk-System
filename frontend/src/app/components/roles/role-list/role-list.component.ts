...
export class RoleListComponent implements OnInit {
  ...
  constructor(private readonly roleService: RoleService, private readonly cdr: ChangeDetectorRef) {}
  ...
  deleteRole(id: number): void { this.roleService.delete(id).subscribe({ next: () => (this.roles = this.roles.filter((item) => item.id !== id)), error: () => (this.errorMessage = 'Unable to delete role.') }); }
}
